...
const optionKey = (option, index = 0) =>
  String(option.id ?? option.key ?? option.slug ?? option.label ?? `discovery-${index}`)

const serviceRef = (option) =>
  option.serviceId ??
  option.service_id ??
  option.service?.id ??
  option.service?.slug ??
  option.serviceSlug ??
  option.service_slug ??
  option.main_service_slug

function toggle(option, index) {
  const key = optionKey(option, index)
  const selectedIndex = selected.value.indexOf(key)
  if (selectedIndex === -1) selected.value.push(key)
  else selected.value.splice(selectedIndex, 1)
}
...
const selectedOptions = computed(() =>
  selected.value
    .map((optionId) => {
      return catalogue.discoveryOptions.find((item, index) => optionKey(item, index) === optionId)
    })
    .filter(Boolean),
)

const keywordSlug = (option) => {
  const text = `${option.label ?? option.title ?? option.name ?? ''}`
  if (/كنب|أريكة|صوفا/.test(text)) return 'sofa'
  if (/سجاد|موكيت/.test(text)) return 'carpet'
  if (/ستائ/.test(text)) return 'curtains'
  if (/مرتب|سرير/.test(text)) return 'mattress'
  if (/حشر|بق/.test(text)) return 'pest'
  if (/شامل|منزل|بيت|شقة|فيلا/.test(text)) return 'apartment'
  return null
}

function resolveOptionService(option, index = 0) {
  const direct = serviceRef(option)
  const byDirect = direct != null ? catalogue.serviceById(direct) : null
  if (byDirect) return byDirect

  const slug = keywordSlug(option)
  const byKeyword = slug ? catalogue.serviceById(slug) : null
  if (byKeyword) return byKeyword

  return catalogue.services.filter((service) => service.featured)[index] ?? catalogue.services[index] ?? null
}

const selectedServices = computed(() =>
  selectedOptions.value
    .map((option) => resolveOptionService(option, catalogue.discoveryOptions.indexOf(option)))
    .filter(Boolean),
)
...
const recommendation = computed(() => {
  ...
  if (selectedServices.value.length >= 3) {
    const selectedRefs = selectedServices.value.flatMap((service) => [String(service.id), service.slug].filter(Boolean))
    const best = catalogue.packages
      .map((pkg) => ({
        pkg,
        hits: pkg.items.filter((item) =>
          selectedRefs.includes(String(item.serviceId)) || selectedRefs.includes(item.bookingSlug),
        ).length,
      }))
  ...
  const service = selectedServices.value[selectedServices.value.length - 1]
      ...
      <ul class="disc__options">
        ...
        >
          <button
            type="button"
            class="disc__option"
            :class="{ 'disc__option--on': isSelected(optionKey(option, index)) }"
            :aria-pressed="isSelected(optionKey(option, index))"
