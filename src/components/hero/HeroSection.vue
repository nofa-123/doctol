...
import heroPhoto from '@/assets/mockup/hero-mobile-clean.png'
...
const heroEyebrow = computed(() => catalogue.homeHero.eyebrow || 'خدمات تنظيف احترافية في السعودية')
const heroDescription = computed(() => catalogue.homeHero.description || 'احجز خدمة التنظيف التي تحتاجها، ونحن نهتم بالباقي — فريق متخصص يصلك بالمعدات كاملة، بسعر واضح من البداية.')
const desktopHeroImage = computed(() => catalogue.homeHero.image || heroImage)
      ...
      <div class="hero__copy">
        ...
        <h1 id="hero-title" data-hero="line" class="hero__title">
          ...
          <span class="hero__title-desktop">
            <template v-if="catalogue.homeHero.title">
              {{ catalogue.homeHero.title }}
            </template>
            <template v-else>
              بيتك يستحق
